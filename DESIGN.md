...
- **Memorable signature:** 첫 화면의 스크롤 스크럽 영상과 화면 오른쪽의 수직 필드 로그가 시계를 정밀 계측 중인 장면처럼 보이게 한다.
...
최대 너비 1440px의 비대칭 구성이 기본이다. 첫 화면은 스크롤 위치에 따라 제품 영상이 감기는 `rd-scrub` 히어로가 담당하고, 이후 데이터 스트립과 사양 행은 실제 제품 정보를 구조화한다. 860px 이하에서는 모든 주요 영역이 단일 열로 바뀌며, 560px 이하에서는 터치 우선 여백과 전폭 CTA를 사용한다. 이미지와 영상 비율을 명시해 로딩 중 레이아웃 이동을 막는다.
...
제품 카탈로그는 `products.json`을 단일 원본으로 사용한다. `group: featured` 제품은 홈 대표 제품으로, `group: other` 제품은 제품 상세 이후의 “MWC의 다른 제품 만나보기” 영역으로 렌더링해 같은 데저트카키 카드가 반복되지 않게 한다. 카드는 고정된 정사각 이미지 영역, 제품명, 가격, 외부 구매 행동을 가지며 로딩·빈 목록·오류 상태에서도 같은 영역을 예약한다.

### Forms and overlays

현재 사이트에는 전송 폼이나 모달이 없다. 연락은 명시적인 고객지원 이메일 링크로 제공한다.
