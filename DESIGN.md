...
제품 카탈로그는 `products.json`을 단일 원본으로 사용한다. 홈은 배열의 앞쪽 3개를 대표 제품으로, 제품 섹션은 전체 배열을 같은 카드 구조로 렌더링한다. 카드는 고정된 정사각 이미지 영역, 제품명, 가격, 외부 구매 행동을 가지며 로딩·빈 목록·오류 상태에서도 같은 영역을 예약한다.
